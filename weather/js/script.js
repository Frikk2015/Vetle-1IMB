...
for (i = 0; i < 24; i++){
    ...
    option.value = i;
    option.text = i;
    document.getElementById("timeSelect").add(option);
}

for (i=0; i < document.getElementById("timeSelect").options.length; i++){
    if (currentHour + i < 10){
        document.getElementById("timeSelect").options[i].innerHTML = '0' + (currentHour + i) + ':00';
    }
    else if (currentHour + i >= 24){
        if (currentHour + i - 24 >= 10){
            document.getElementById("timeSelect").options[i].innerHTML = (currentHour + i - 24) + ':00';
        }
        else{
            document.getElementById("timeSelect").options[i].innerHTML = '0' + (currentHour + i - 24) + ':00';
        }
    }
    else {
        document.getElementById("timeSelect").options[i].innerHTML = (currentHour + i) + ':00';
    }
}